...
namespace GvcRevitPlugins.TerrainCheck.Rules
{
    public static class CheckRules
    {
        ...
        public static void Execute(UIDocument uidoc, XYZ[] startPoints, XYZ normal, XYZ[] boundaryPoints, double baseElevation, bool draw)
        {
            foreach (ITerrainCheckRule rule in Rules)
                if (rule.IsActive)
                    rule.Execute(uidoc, startPoints, normal, boundaryPoints, baseElevation, draw);
        }
    }
}
